...
# sudo chown root:root -R f2b
# sudo chmod 644 -R f2b
...
# 1) COPY FILES
sudo cp -v ./jail.local /etc/fail2ban/jail.local
sudo cp -v ./filter.d/*.conf /etc/fail2ban/filter.d/
echo "STEP 1 COMPLETED: Copied jail.local and filter.d/*.conf to /etc/fail2ban/"

# 2) UPDATE LOGSDIR IN JAIL.LOCAL
sudo sed -i "s|logsdir[[:space:]]*=[[:space:]]*.*|logsdir = $LOG_DIR|g" /etc/fail2ban/jail.local
echo "STEP 2 COMPLETED: Updated logsdir in /etc/fail2ban/jail.local to $LOG_DIR"

# 3) COPY CONF FILES
sudo cp -v /etc/fail2ban/action.d/iptables.conf /etc/fail2ban/action.d/iptables-forward.conf
sudo cp -v /etc/fail2ban/action.d/iptables-allports.conf /etc/fail2ban/action.d/iptables-allports-forward.conf
sudo cp -v /etc/fail2ban/action.d/iptables-multiport.conf /etc/fail2ban/action.d/iptables-multiport-forward.conf
echo "STEP 3 COMPLETED: Copied and renamed iptables action files in /etc/fail2ban/action.d/"

# 4) REPLACE NAMES IN ACTION FILES
sudo sed -i 's/chain[[:space:]]*=[[:space:]]*INPUT/chain=DOCKER-USER/g' /etc/fail2ban/action.d/iptables-forward.conf
sudo sed -i 's/iptables.conf/iptables-forward.conf/g' /etc/fail2ban/action.d/iptables-multiport-forward.conf
sudo sed -i 's/iptables.conf/iptables-forward.conf/g' /etc/fail2ban/action.d/iptables-allports-forward.conf
